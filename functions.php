...
/**
 * 호스트 문자열의 punycode/유니코드 변형 목록.
 *
 * @param string $host 호스트명.
 * @return string[]
 */
function della_theme_host_variants( $host ) {
	$host     = strtolower( trim( $host ) );
	$variants = $host !== '' ? array( $host ) : array();
	if ( $host === '' ) {
		return $variants;
	}
	if ( function_exists( 'idn_to_ascii' ) ) {
		...
		if ( is_string( $ascii ) && $ascii !== '' ) {
			$variants[] = strtolower( $ascii );
		}
	}
	if ( function_exists( 'idn_to_utf8' ) ) {
		$utf8 = idn_to_utf8( $host, IDNA_DEFAULT, INTL_IDNA_VARIANT_UTS46 );
		if ( is_string( $utf8 ) && $utf8 !== '' ) {
			$variants[] = strtolower( $utf8 );
		}
	}
...
function della_theme_get_internal_site_hosts() {
	...
	foreach ( $candidates as $url ) {
		...
		if ( is_string( $h ) && $h !== '' ) {
			$hosts = array_merge( $hosts, della_theme_host_variants( $h ) );
		}
	}
	$hosts = array_values( array_unique( $hosts ) );
	$hosts = apply_filters( 'della_theme_internal_site_hosts', $hosts );
	return $hosts;
}
...
/**
 * 내부 링크 URL (경로 또는 절대 URL → 현재 접속 호스트).
 *
 * @param string $path_or_url home_url 경로, 또는 이미 생성된 permalink.
 * @return string
 */
function della_theme_internal_url( $path_or_url = '/' ) {
	if ( is_string( $path_or_url ) && preg_match( '#^https?://#i', $path_or_url ) ) {
		return della_theme_preserve_current_host_url( $path_or_url );
	}
	if ( is_string( $path_or_url ) && isset( $path_or_url[0] ) && $path_or_url[0] === '/' ) {
		return della_theme_preserve_current_host_url( $path_or_url );
	}
	return della_theme_preserve_current_host_url( home_url( $path_or_url ) );
}

/**
 * 내부 링크 esc_url (현재 호스트 유지).
 *
 * @param string $url URL.
 * @return string
 */
function della_theme_esc_internal_url( $url ) {
	return esc_url( della_theme_preserve_current_host_url( $url ) );
}

/**
 * 본문 HTML 안의 사이트 절대 URL을 현재 호스트로 치환 (DB 저장 링크 대응).
 *
 * @param string $content post_content 등.
 * @return string
 */
function della_theme_preserve_current_host_in_content( $content ) {
	if ( ! della_theme_should_preserve_current_host() || ! is_string( $content ) || $content === '' ) {
		return $content;
	}
	if ( strpos( $content, 'http' ) === false && strpos( $content, 'href' ) === false ) {
		return $content;
	}
	$origin = della_theme_current_origin();
	if ( $origin === '' ) {
		return $content;
	}
	$origin = rtrim( $origin, '/' );
	foreach ( della_theme_get_internal_site_hosts() as $host ) {
		$content = preg_replace( '#https?://' . preg_quote( $host, '#' ) . '#i', $origin, $content );
	}
	return $content;
}

/**
 * wp_nav_menu 항목 URL 현재 호스트 유지.
 *
 * @param WP_Post[] $items 메뉴 항목.
 * @return WP_Post[]
 */
function della_theme_nav_menu_objects_preserve_host( $items ) {
	if ( ! della_theme_should_preserve_current_host() || ! is_array( $items ) ) {
		return $items;
	}
	foreach ( $items as $item ) {
		if ( isset( $item->url ) && is_string( $item->url ) && $item->url !== '' ) {
			$item->url = della_theme_preserve_current_host_url( $item->url );
		}
	}
	return $items;
}
...
/**
 * post/page/term/home_url/the_permalink 등 내부 링크를 현재 호스트로 통일.
 */
function della_theme_register_current_host_url_filters() {
	...
	foreach ( $link_filters as $filter_name ) {
		add_filter( $filter_name, 'della_theme_filter_preserve_current_host_url', $priority );
	}
	add_filter( 'della_hero_home_url', 'della_theme_filter_preserve_current_host_url', $priority );
	add_filter( 'wp_nav_menu_objects', 'della_theme_nav_menu_objects_preserve_host', $priority );
	add_filter( 'the_content', 'della_theme_preserve_current_host_in_content', $priority );
	add_filter( 'widget_text_content', 'della_theme_preserve_current_host_in_content', $priority );

	// AIOSEO 브레드크럼·기타 링크 (있을 때만 적용).
	add_filter( 'aioseo_breadcrumbs_link', 'della_theme_filter_preserve_current_host_url', $priority );
	add_filter( 'aioseo_breadcrumb_url', 'della_theme_filter_preserve_current_host_url', $priority );

	// 멀티 도메인 별칭 (동일 사이트).
	add_filter(
		'della_theme_internal_site_hosts',
		static function ( $hosts ) {
			foreach ( array( 'suwon-lawyer.com', 'www.suwon-lawyer.com' ) as $alias ) {
				$hosts = array_merge( $hosts, della_theme_host_variants( $alias ) );
			}
...
function della_theme_consultation_url() {
	...
	if ( is_string( $url ) && trim( $url ) !== '' ) {
		return della_theme_preserve_current_host_url( trim( $url ) );
	}
	return della_theme_internal_url( '/' );
}

/** 상호(법인명) — 전 구역 통일용. */
function della_theme_firm_name() {
	return get_theme_mod( 'della_firm_name', '법무법인 동주' );
}
...
function della_theme_document_title_parts( $parts ) {
	...
	if ( is_front_page() ) {
		$parts['title'] = '수원 성범죄 전문변호사 | 강제추행·카메라촬영 대응 | ' . della_theme_firm_name();
		unset( $parts['tagline'], $parts['site'], $parts['page'] );
		return $parts;
	}
	...
	if ( $path === 'success-cases' || strpos( $path, 'success-cases/' ) === 0 || in_array( $path, array( 'case', 'success', '성범죄-성공사례', '성공사례' ), true ) ) {
		$parts['title'] = della_theme_trim_document_title( '수원 성범죄 성공사례 | 강제추행·카메라촬영 무혐의·기소유예 사례 | ' . della_theme_firm_name() );
		unset( $parts['tagline'], $parts['site'], $parts['page'] );
		return $parts;
	}
	if ( della_theme_is_lawyers_page() ) {
		...
		if ( $lawyer_slug && function_exists( 'della_theme_get_lawyer_by_slug' ) ) {
			...
			if ( $lawyer && ! empty( $lawyer['name'] ) ) {
				$parts['title'] = della_theme_trim_document_title( $lawyer['name'] . ' 변호사 프로필 | 수원 성범죄 전문변호사 | ' . della_theme_firm_name() );
				unset( $parts['tagline'], $parts['site'], $parts['page'] );
				return $parts;
			}
		}
	...
	if ( della_theme_is_response_board_page() ) {
		$parts['title'] = della_theme_trim_document_title( '성범죄 대응정보 가이드 | 강제추행·불법촬영·아청법·판례·FAQ | ' . della_theme_firm_name() );
		unset( $parts['tagline'], $parts['site'], $parts['page'] );
		return $parts;
	}
...
add_filter( 'aioseo_disable', 'della_theme_disable_aioseo_on_front_page', 1 );
...
function della_theme_get_home_og_image_url() {
	$ud     = wp_upload_dir();
	$og_url = '';
	if ( ! empty( $ud['basedir'] ) && ! empty( $ud['baseurl'] ) ) {
	...
	if ( ! $og_url && function_exists( 'get_template_directory' ) && function_exists( 'get_template_directory_uri' ) ) {
		...
		if ( file_exists( $theme_og ) ) {
			$og_url = get_template_directory_uri() . '/assets/og/dongju-sexcrime-lawyer-1200x630.jpg';
		}
	}
	if ( ! $og_url ) {
...
/**
 * SEO: 홈페이지 전용 메타 (title, description, robots, canonical, OG, Twitter).
...
function della_theme_homepage_seo_meta() {
	...
	$title       = '수원 성범죄 전문변호사 | 강제추행·카메라촬영 대응 변호사 | 법무법인 동주';
	$description = '수원 성범죄 전문변호사 법무법인 동주. 강제추행·카메라촬영·디지털 성범죄 사건 대응. 중앙지검 부장검사·국정원·경찰청 경력 변호사가 고소 전 합의부터 경찰조사, 검찰송치, 재판까지 직접 대응합니다. 수원 광교.';
	$robots      = 'index,follow,max-image-preview:large,max-snippet:-1,max-video-preview:-1';

	$og_image_type = 'image/jpeg';
	if ( strpos( $og_url, '.webp' ) !== false ) {
		$og_image_type = 'image/webp';
	} elseif ( strpos( $og_url, '.png' ) !== false ) {
...
<meta property="og:image:alt" content="<?php echo esc_attr( $og_alt ); ?>">

<!-- Twitter -->
<meta name="twitter:card" content="summary_large_image">
<?php if ( is_string( $twitter_site ) && trim( $twitter_site ) !== '' ) : ?>
<meta name="twitter:site" content="<?php echo esc_attr( trim( $twitter_site ) ); ?>">
<?php endif; ?>
<meta name="twitter:title" content="<?php echo esc_attr( $title ); ?>">
<meta name="twitter:description" content="<?php echo esc_attr( $description ); ?>">
<meta name="twitter:image" content="<?php echo esc_url( $og_url ); ?>">
<meta name="twitter:image:alt" content="<?php echo esc_attr( $og_alt ); ?>">
	<?php
}
 ...
 *
 * @return string[] 유효한 URL만 반환
 */
function della_theme_schema_same_as_urls() {
	...
	return apply_filters( 'della_theme_schema_same_as_urls', $urls );
}
...
function della_theme_front_page_meta_100() {
	...
	$same_as = function_exists( 'della_theme_schema_same_as_urls' ) ? della_theme_schema_same_as_urls() : array();
	...
	if ( is_string( $postal_code ) && trim( $postal_code ) !== '' ) {
		$org_address['postalCode'] = trim( $postal_code );
	}
	...
	if ( is_string( $postal_code ) && trim( $postal_code ) !== '' ) {
		$legal_service['address']['postalCode'] = trim( $postal_code );
	}
	if ( ! empty( $same_as ) ) {
		$legal_service['sameAs'] = $same_as;
	}
	if ( is_string( $has_map ) && trim( $has_map ) !== '' ) {
		$legal_service['hasMap'] = $has_map;
	}
...
function della_theme_front_page_description() {
	...
	$firm = della_theme_firm_name();
	return __( '수원성범죄변호사, 수원성범죄전문변호사 ', 'della-theme' ) . $firm . __( ' 수원 성범죄연구센터. 성범죄피해자변호사·형사전문변호사 상담. 수원 광교 신속 상담.', 'della-theme' );
}
...
function della_theme_customize_register( $wp_customize ) {
	...
	$wp_customize->add_setting( 'della_firm_name', array(
		'default'           => '법무법인 동주',
		'sanitize_callback' => 'sanitize_text_field',
	) );
	$wp_customize->add_control( 'della_firm_name', array(
		'label'   => __( '상호 (법인명)', 'della-theme' ),
		'section' => 'della_directions_section',
		'type'    => 'text',
	) );
...
function della_theme_lawyers_page_url() {
	...
	if ( $cached_url !== null ) {
		return della_theme_preserve_current_host_url( $cached_url );
	}
	$pages = get_pages( array(
		'meta_key'   => '_wp_page_template',
		'meta_value' => 'page-lawyers.php',
...
function della_theme_lawyer_profile_url( $slug ) {
	...
	$url  = $base . '/' . sanitize_file_name( $slug ) . '/';
	return della_theme_preserve_current_host_url( $url );
}
...
function della_theme_get_fallback_description() {
	...
	if ( is_front_page() ) {
		return '수원 성범죄 전문변호사 ' . della_theme_firm_name() . '. 중앙지검 부장검사·국정원·경찰청 경력 변호사가 고소 전 합의부터 경찰조사, 검찰송치, 재판까지 직접 대응합니다. 수원 광교.';
	}
	...
	if ( $path === 'success-cases' || strpos( $path, 'success-cases/' ) === 0 || strpos( $path, '/success-cases' ) !== false || in_array( $path, array( 'case', 'success', '성범죄-성공사례', '성공사례' ), true ) ) {
		return '수원 성범죄 사건 성공사례를 확인하세요. 강제추행·카메라촬영·아청법 사건에서 무혐의·기소유예·집행유예 등 실제 결과를 공개합니다. 경찰조사부터 재판까지 ' . della_theme_firm_name() . '가 직접 대응합니다.';
	}
	...
	if ( function_exists( 'della_theme_is_lawyers_page' ) && della_theme_is_lawyers_page() && ! get_query_var( 'lawyer_slug' ) ) {
		return della_theme_firm_name() . ' 성범죄 전문변호사 팀을 소개합니다. 형사법 전문 변호사들이 성범죄 사건에서 경찰조사·검찰송치·재판까지 단계별 대응 전략을 체계적으로 설계합니다. 상담 전 변호사 경력과 전문분야를 확인하세요.';
	}
	// 대응정보 허브
...
/**
 * SEO: Robots 메타 content 값 — index,follow 명시 (테마에서 1회만 출력할 때 사용).
 *
 * @return string
 */
function della_theme_robots_content() {
	return apply_filters( 'della_theme_robots_content', 'index,follow,max-image-preview:large' );
}

/**
 * SEO: Robots meta — header.php에서 index,follow 1회만 출력. AIOSEO가 404/검색 등 특수 케이스 처리.
 */
...
function della_theme_schema_json_ld() {
	...
	if ( is_singular( 'post' ) ) {
		...
		if ( $post instanceof WP_Post ) {
			...
			$publisher_logo = function_exists( 'della_theme_get_site_logo_url' ) ? della_theme_get_site_logo_url() : '';
			$schema['publisher'] = array(
				'@type' => 'Organization',
				'name'  => get_bloginfo( 'name' ),
				'url'   => home_url( '/' ),
			);
			if ( is_string( $publisher_logo ) && trim( $publisher_logo ) !== '' ) {
	...
	if ( della_theme_is_success_cases_page() ) {
		...
		$sc_email = get_theme_mod( 'della_contact_email', 'dongjucriminal@gmail.com' );
		$legal_service = array(
			'@type'        => 'LegalService',
			...
			'url'          => $base_url,
			'logo'         => $logo_url,
			'image'        => $og_image ? array( $og_image ) : array( $logo_url ),
			'description'  => '수원 성범죄 전문변호사 ' . della_theme_firm_name() . '. 강제추행·카메라촬영·디지털 성범죄 사건 대응. 성공사례·대응정보 제공.',
			'email'        => is_email( $sc_email ) ? $sc_email : '',
			'priceRange'   => '상담 문의',
			'areaServed'   => array(
		...
		if ( is_string( $postal_code ) && trim( $postal_code ) !== '' ) {
			$sc_org_address['postalCode'] = trim( $postal_code );
		}
		$sc_organization = array(
			'@type'      => 'Organization',
			...
			'email'      => is_email( get_theme_mod( 'della_contact_email', '' ) ) ? get_theme_mod( 'della_contact_email', '' ) : '',
			'address'    => $sc_org_address,
		);
		if ( ! empty( $same_as ) ) {
			$sc_organization['sameAs'] = $same_as;
		}
		...
		$graph = array(
			...
			array(
				...
				'url'        => $sc_page_url,
				'name'       => '수원 성범죄 성공사례 | ' . della_theme_firm_name(),
	...
	// Hub 구조 내부링크 ItemList (front page 전용, 사이트맵 중복 제거)
	if ( is_front_page() ) {
		...
		$consult_url   = function_exists( 'della_theme_consultation_url' ) ? della_theme_consultation_url() : home_url( '/' );
		$base_hub      = home_url( '/' );
		$hub_nav_items = array(
			...
			array( 'name' => __( '상담 신청', 'della-theme' ), 'url' => $consult_url ),
			array( 'name' => __( '오시는 길', 'della-theme' ), 'url' => $base_hub . '#directions' ),
		);
		$hub_schema = array(
			'@context'        => 'https://schema.org',
			'@type'           => 'ItemList',
			'name'            => __( '성범죄 전문 허브 내부 링크', 'della-theme' ),
			'description'     => __( '수원 성범죄 변호사 허브 메인 내 섹션 및 대응정보·성공사례·상담·오시는 길 링크', 'della-theme' ),
			'itemListElement' => array(),
		);
...
function della_theme_content_images_alt_title( $content ) {
	...
	$context    = __( '수원 성범죄 전문 변호사', 'della-theme' ) . ' ' . della_theme_firm_name();
